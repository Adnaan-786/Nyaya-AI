"""Deterministic, heuristic completions for the `fake` LLM provider.

Every prompt built for a structured AI task starts its *system* message with a
literal `TASK: <name>` tag. This module reads that tag and fabricates a
plausible, schema-correct response using simple text heuristics over the *user*
message, which carries the actual document or query text — no LLM call, no
network, no key.

Fabricating a response the caller then parses (rather than short-circuiting the
parse with a canned object) is the point: the real JSON-handling and validation
paths run in FAKE_MODE too, so a schema mistake surfaces offline instead of on
the first live call. Same reasoning as the eCourts and OCR fixtures.
"""
...
def _normalize_legal_query(query: str) -> dict:
    ...
    return {
        # No translation offline; a live provider is what turns Hinglish into English.
        "english_query": query,
        "sections": sections,
        "acts": acts,
        "years": years,
...
def _research_generate(payload_json: str) -> dict:
    ...
    if not fragments:
        return {
            "answer_markdown": (
                f"No relevant authority was found in the available sources for: {query}"
            ),
            "citations": [],
        }
...
def _fake_draft_markdown(template_id: str, fields_json: str) -> str:
    """Renders the caller's own fields as a labelled Markdown document.

    Not prose, but every word of it comes from the fields — never an invented fact —
    and it exercises the same Markdown-to-DOCX path a real completion would.
    """
    try:
    ...
    lines.append("")
    lines.append(
        "*This is a FAKE_MODE draft assembled directly from the fields above. "
        "Configure a real LLM provider for generated prose.*"
    )
    return "\n".join(lines)
...
def fake_complete(*, system: str, user: str) -> str:
    ...
    # An untagged or future task still gets something parseable as prose, clearly
    # marked so it can never be mistaken for a real answer in a screenshot.
