"""
Draftsman template catalog (plan C.9): 12 launch templates, each a
typed field schema + prose-generation instructions for the LLM. The
LLM fills prose sections from the fields + case data; it never invents
facts not present in the fields (enforced by prompt instruction and by
`missing_fields` reporting rather than silent guessing -- see
...
class TemplateField:
    name: str
    label: str
    type: str = "string"  # string | date | number | text (multi-line)
    required: bool = True
...
class Template:
    ...
    instructions: str  # guidance the LLM uses to draft prose sections
...
TEMPLATES: dict[str, Template] = {
    "bail_application": Template(
        ...
        instructions=(
            ...
            "title/cause-title block, brief facts, grounds for bail (expand "
            "the given grounds into a persuasive but factual argument), and "
            "a prayer clause requesting bail on such terms as the court "
            "deems fit."
        ),
    ),
    "legal_notice_138": Template(
        ...
        instructions=(
            "Draft a statutory legal notice under Section 138 of the "
            "Negotiable Instruments Act. Must state the cheque particulars, "
            "the fact and date of dishonor, and demand payment of the "
            "cheque amount within 15 days of receipt, failing which legal "
    ...
    "vakalatnama": Template(
        ...
        instructions=(
            "Draft a vakalatnama authorizing the named advocate to appear, "
            "plead, and act on behalf of the client in the specified case "
            "before the named court, including the usual powers to file "
            "applications, receive notices, and compromise with instructions."
        ),
    ),
    "plaint": Template(
        id="plaint",
        ...
        fields=[
            ...
            TemplateField("valuation_paise", "Suit valuation (paise)", type="number", required=False),
        ],
        instructions=(
            "Draft a plaint skeleton: cause-title, parties, jurisdiction "
            "statement, facts constituting the cause of action, valuation "
            "for court fees, and the prayer clause with the relief sought."
        ),
    ),
    "written_statement": Template(
        ...
        fields=[
            ...
            TemplateField("preliminary_objections", "Preliminary objections", type="text", required=False),
        ],
        instructions=(
            "Draft a written statement responding paragraph-wise in spirit "
            "to the plaint: preliminary objections (if any), a para-wise "
            "reply covering the given response points, and a prayer for "
    ...
    "rti_application": Template(
        ...
        instructions=(
            "Draft an application under the Right to Information Act, 2005 "
            "addressed to the Public Information Officer, clearly listing "
            "the information sought and requesting it within the statutory "
            "30-day period."
        ),
    ),
    "consumer_complaint": Template(
        ...
        fields=[
            ...
            TemplateField("deficiency_details", "Deficiency in service / defect details", type="text"),
            TemplateField("relief_sought", "Relief sought", type="text"),
        ],
        instructions=(
            "Draft a consumer complaint under the Consumer Protection Act, "
            "2019: parties, facts describing the deficiency in service or "
            "defective goods, jurisdiction, and the relief/compensation sought."
        ),
    ),
    "affidavit": Template(
        id="affidavit",
        ...
        instructions=(
            "Draft a general affidavit: deponent's identity, a numbered "
            "statement of the facts to be affirmed on solemn affirmation, "
            "and a verification clause."
        ),
    ),
    "reply_to_legal_notice": Template(
        ...
        instructions=(
            "Draft a reply to a legal notice: acknowledge receipt and date "
            "of the original notice, respond point-by-point to its "
            "allegations using the given response points, and deny any "
    ...
    "maintenance_petition": Template(
        ...
        fields=[
            ...
            TemplateField("petitioner_income_details", "Petitioner's income/needs", type="text"),
            TemplateField("respondent_income_details", "Respondent's income (if known)", type="text", required=False),
            TemplateField("maintenance_amount_sought_paise", "Maintenance amount sought (paise)", type="number"),
        ],
        instructions=(
            "Draft a maintenance petition stating the relationship between "
            "the parties, the petitioner's financial needs and the "
            "respondent's means (as known), and the monthly maintenance "
            "amount sought."
        ),
    ),
    "anticipatory_bail": Template(
        ...
        fields=[
            TemplateField("court_name", "Court name"),
            TemplateField("case_number", "Case / FIR number (if registered)", required=False),
            TemplateField("applicant_name", "Applicant name"),
            TemplateField("sections_apprehended", "Sections apprehended"),
            TemplateField("apprehension_grounds", "Grounds for apprehension of arrest", type="text"),
        ],
        instructions=(
            "Draft an anticipatory bail application under Section 438 "
            "CrPC/482 BNSS: grounds for the apprehension of arrest, that "
            "the applicant is not a flight risk and will cooperate with "
            "investigation, and a prayer for pre-arrest bail."
        ),
    ),
    "adjournment_application": Template(
        ...
        instructions=(
            "Draft a short application seeking adjournment of the hearing, "
            "stating the reason and (if given) the next date sought, "
            "undertaking not to seek further unnecessary adjournments."
        ),
    ),
}
